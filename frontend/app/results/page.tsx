...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    setResult(r);
    // Mini tests for the active week never spawn a new roadmap — the user
    // keeps the roadmap chosen right after signup. Only the entrance test
    // (no topic / no mini flag) generates a fresh roadmap.
    if (!r.is_mini_test) {
      generateRoadmap(r);
    }
  ...
  return (
    <div className="min-h-screen pb-10 noise-bg" style={{ background: "#07080f" }}>
      <div className="max-w-2xl mx-auto px-4 pt-8">
        ...
        {/* Mini-test mode: no new roadmap, just a focused recap + nudge */}
        {result.is_mini_test ? (
          <div className="space-y-4">
            <div className="glass rounded-3xl p-6 relative overflow-hidden" style={cardStyle(150)}>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  {result.week !== undefined && result.week !== null && (
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>Tuần {result.week}</p>
                  )}
                </div>
              </div>
              ...
              {score >= 8.5 ? (
                ...
                <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
                  Bạn đang gần đạt rồi! Hãy ôn lại <strong style={{ color: "#fff" }}>{result.topic_name || "chủ đề này"}</strong> và làm lại Mini Test khi sẵn sàng. Lộ trình của bạn không thay đổi — chỉ cần luyện thêm.
                </p>
              ) : (
                <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
                  Chủ đề <strong style={{ color: "#fff" }}>{result.topic_name || "này"}</strong> còn yếu. Hãy quay lại học kỹ phần lý thuyết của tuần này rồi thử lại Mini Test. Lộ trình của bạn vẫn giữ nguyên.
                </p>
              )}
              ...
              {result.weak_topics?.length > 0 && score < 7 && (
                <div className="mt-4 p-3 rounded-2xl"
                  style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)" }}>
                  <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "#f87171" }}>
                    Điểm yếu phát hiện
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {result.weak_topics.map((t: string) => (
            ...
            <div className="grid grid-cols-2 gap-3" style={cardStyle(250)}>
              <button onClick={() => router.push("/test")}
                className="py-4 font-bold text-white rounded-2xl text-sm transition-all"
                style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}>
                Làm lại Mini Test
              </button>
              <button onClick={() => router.push("/dashboard")}
                className="py-4 font-black text-white rounded-2xl text-sm transition-all"
          ...
          <div className="glass rounded-3xl p-8 text-center mb-5" style={cardStyle(100)}>
            ...
            <p className="text-white font-bold mb-2">Routex đang tạo lộ trình cá nhân hóa...</p>
            <p className="text-sm mb-5" style={{ color: "var(--text-secondary)" }}>Routex đang phân tích kết quả của bạn</p>
                    ...
                    return (
                      <div key={i} className="rounded-3xl p-5 relative overflow-hidden transition-all duration-300"
                        ...
                        <div className="rounded-2xl p-3 mb-4 text-center"
                          ...
                          <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "var(--text-muted)" }}>Routex dự đoán điểm</p>
