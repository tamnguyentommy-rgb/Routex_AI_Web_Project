  ...
  useEffect(() => {
    ...
    const ping = async () => {
      try {
        ...
        if (!done && elapsed < 3000) {
          setWarming(false);
        }
      } catch {
        // silent — server will wake up on login attempt
      } finally {
        done = true;
        setWarming(false);
      }
    ...
    // Only show "warming up" banner if server takes more than 2s to respond
    timer = setTimeout(() => {
      if (!done) setWarming(true);
    }, 2000);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden noise-bg"
      ...
      <div className="relative z-10 w-full max-w-sm px-4">
        {/* Warming up banner */}
        {warming && (
          <div className="mb-4 flex items-center gap-2 px-4 py-3 rounded-xl text-amber-300 text-xs animate-fade-up"
            style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)" }}>
            <span className="w-3 h-3 rounded-full border-2 border-amber-300/40 border-t-amber-300 inline-block flex-shrink-0"
        ...
        <div className="text-center mb-10 animate-fade-up">
          ...
          <h1 className="text-3xl font-black text-white tracking-tight mb-1">Routex <span className="gradient-text"></span></h1>
          <p style={{ color: "var(--text-secondary)" }} className="text-sm">Lộ trình học thông minh</p>
        </div>
        ...
        <div className="glass rounded-3xl p-7 shadow-2xl animate-fade-up" style={{ animationDelay: "0.1s" }}>
          ...
          <form onSubmit={handle} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              ...
              onMouseOver={e => !loading && !warming && ((e.target as HTMLElement).style.transform = "translateY(-1px)", (e.target as HTMLElement).style.boxShadow = "0 12px 40px rgba(99,102,241,0.4)")}
