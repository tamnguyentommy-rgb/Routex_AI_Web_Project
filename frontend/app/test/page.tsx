...
export default function TestPage() {
  ...
  const loadTest = async (
    ...
    try {
      const res = tp
        ? await api.generateMiniTest(subj, gr, tp, user!.id, wk ?? undefined)
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {isMiniTest && (
                <p className="text-xs font-semibold" style={{ color: "#a78bfa" }}>Mini Test: {topic}</p>
              )}
              {!isMiniTest && (
